...
def get_binding_potential_energy(filename):
    ...
    #plt.plot(times, potential_energies, 'b')
    plt.plot(times, potential_energies_c, 'b')
    plt.plot(times, potential_energies_r, 'g')
    plt.plot(times, potential_energies_l, 'r')
    plt.ylabel('U ({})'.format(potential_energies.unit))
    plt.xlabel('time ({})'.format(saving_time.unit))
    plt.legend(["Complex", "Receptor", "Ligand"], loc ="lower right")
    plt.show()
